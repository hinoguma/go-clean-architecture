...
func NewAdapterErrorValidation(errs []ValidateError) AdapterError {
	details := make([]utils.ValidateErrorDetail, 0, len(errs))
	for _, ve := range errs {
		details = append(details, ve.UtilValidateErrorDetail())
	}
	return AdapterError{
		ErrType:        ErrorTypeValidation,
		Err:            utils.NewValidationFailedError(details),
		ValidateErrors: errs,
	}
}
...
func (ve ValidateError) UtilValidateErrorDetail() utils.ValidateErrorDetail {
	return utils.ValidateErrorDetail{
		Field:   ve.Field,
		Message: ve.Message,
	}
}
