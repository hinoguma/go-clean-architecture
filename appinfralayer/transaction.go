...
func (tl transactionList) add(tr transactionProcess) error {
	...
	if ok {
		err := utils.NewError("transaction already exists")
...
func NewSQLTransactionFactory() infrainterface.TransactionFactory {
...
func (factory sqlTransactionFactory) BeginTx(ctx context.Context, opt model.TxOptions) (context.Context, infrainterface.Transaction, error) {
...
func NewTransactionNotFoundError(transactionId string) error {
	err := utils.NewError(fmt.Sprintf("transaction not found"))
...
type commonPropsOfLog struct {
	...
	Time          utils.Ymd_Hms_ms   `json:"timestamp"`
}
...
func (log startOperationLog) JsonString() (utils.JsonString, error) {
	b, err := json.Marshal(log)
	if err != nil {
		return "{}", utils.ErrWrap(err, utils.NewError("failed to marshal startOperationLog"))
	}
	return utils.JsonString(b), nil
}

func newStartOperationLog(txId string, opId string, opType operationType, tn string, t utils.Ymd_Hms_ms) startOperationLog {
...
func newCompleteOperationLog(startOpLog startOperationLog, isSuccess bool, t utils.Ymd_Hms_ms) completeOperationLog {
...
func (v completeOperationLog) JsonString() (utils.JsonString, error) {
	b, err := json.Marshal(v)
	if err != nil {
		return "{}", utils.ErrWrap(err, utils.NewError("failed to marshal completeOperationLog"))
	}
	return utils.JsonString(b), nil
}
...
type startQueryLog struct {
	...
	ItemData          utils.JsonString         `json:"ItemData,omitempty"`
}

func newStartQueryLog(
	startOpLog startOperationLog, qId string, qType operationType, tn string, t utils.Ymd_Hms_ms) *startQueryLog {
...
func (v *startQueryLog) WithItemData(data utils.JsonString) *startQueryLog {
	v.ItemData = data
	return v
}

func (v startQueryLog) JsonString() (utils.JsonString, error) {
	b, err := json.Marshal(v)
	if err != nil {
		return "{}", utils.ErrWrap(err, utils.NewError("failed to marshal queryResultLog"))
	}
	return utils.JsonString(b), nil
}
...
func newQueryResultLog(
	startQueryLog startQueryLog, isSuccess bool, t utils.Ymd_Hms_ms,
) *queryResultLog {
...
func (v *queryResultLog) WithItemData(data utils.JsonString) *queryResultLog {
	v.ItemData = &data
	return v
}
...
func (v queryResultLog) JsonString() (utils.JsonString, error) {
	b, err := json.Marshal(v)
	if err != nil {
		return "{}", utils.ErrWrap(err, utils.NewError("failed to marshal queryResultLog"))
	}
...
func (v *undoOperation) WithItemData(data utils.JsonString) *undoOperation {
	v.ItemData = &data
	return v
}
...
func (v undoOperation) JsonString() (utils.JsonString, error) {
	b, err := json.Marshal(v)
	if err != nil {
		return "{}", utils.ErrWrap(err, utils.NewError("failed to marshal queryResultLog"))
	}
...
func (txLogger updateOperationTxLogger) startOperationLog() error {
	...
	fileLogger.Info(txLogger.ctx, utils.NewLogRequest(message.String()).ToValue())
	return nil
}
...
func queryResultFailedLog(
	...
	if utils.IsDataNotFoundError(err) {
		log = log.WithErrorType(operationErrorTypeNotFoundData)
	} else if utils.IsDataLockedError(err) {
		log = log.WithErrorType(operationErrorTypeDataLocked)
	} else if utils.IsConditionNotMatchError(err) {
	...
	fileLogger.Info(ctx, utils.NewLogRequest(message.String()).ToValue())
	return nil
}
...
func (txLogger getQueryLogger) queryResultLog(
	startLog startQueryLog, item utils.JsonString,
) error {
	log := newQueryResultLog(
...
func newUpdateItemRequestForLock(
	txId string, ts utils.UnixTimestamp, isolationLevel string, updatedAt utils.UnixTimestamp,
